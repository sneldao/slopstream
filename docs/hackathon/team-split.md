...
- Publish persisted marketplace events (`bid.placed`, `bid.outbid`, `leaderboard.updated`, `attention.verified`, `bid.cleared`, `bid.uncleared`, `bid.failed`, `reward.pool.updated`, `stats.updated`) via Redis pub/sub. The orchestrator (Lane 3) owns only WebSocket delivery; it does not make marketplace state authoritative
...
- Stream orchestrator: queue manager, segment scheduler, challenge timing (when to fire, not what they are), The Continuum continuity ([architecture](../technical/architecture.md)). Consumes auction results from Lane 2 — the orchestrator never resolves auctions; the ledger is the single source of truth.
...
- **`WsEvent`** — the discriminated union of all **public** WebSocket business events (15 event types: `bid.placed`, `bid.outbid`, `leaderboard.updated`, `auction.opened`, `segment.generating`, `generation.progress`, `segment.ready`, `segment.playing`, `challenge.fired`, `attention.verified`, `bid.cleared`, `bid.uncleared`, `bid.failed`, `reward.pool.updated`, `stats.updated`). This is the public live-event contract between orchestrator/API and every screen.
