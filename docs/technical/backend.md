...
`StreamOpsMetrics` (generation latency, playback, queue depth, ready-buffer
seconds) for the local HUD and optional operational alert dispatcher.
`GET /metrics` exposes the same snapshot as Prometheus text. `queue.snapshotAvailable`
...
The orchestrator gateway answers local HTTP routes before reverse-proxying
everything else to Lane 2: `GET /health`, `GET /stream/snapshot` (stamps
`asOfSequence` into the gateway sequence space), `GET /ops/metrics` (JSON), and
`GET /metrics` (Prometheus text of the same snapshot).
