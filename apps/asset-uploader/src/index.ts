...
const CONTENT_ADDRESSED_AUDIO_KEY = /^audio\/([a-f0-9]{64})\.mp3$/;
...
function objectKeyFromRequest(url: URL): string | undefined {
  ...
  try {
    ...
    return CONTENT_ADDRESSED_AUDIO_KEY.test(objectKey) ? objectKey : undefined;
  } catch {
    return undefined;
  }
}

function sha256ForObjectKey(objectKey: string): string | undefined {
  return CONTENT_ADDRESSED_AUDIO_KEY.exec(objectKey)?.[1];
}
...
/**
 * Authenticated, content-addressed write boundary for public audio. The
...
export default {
  async fetch(request, env): Promise<Response> {
    ...
    const expectedSha256 = objectKey && sha256ForObjectKey(objectKey);
    if (!objectKey || !expectedSha256) {
    ...
    if (request.headers.get("x-content-sha256") !== expectedSha256) {
    ...
    try {
      ...
      if (existing) {
        if (existing.customMetadata?.sha256 !== expectedSha256) {
